...
bgGraphic.drawRect(0, 0, cWidth + 1, cHeight + 1); // 1 as x offset
bgGraphic.endFill();
...
graphContainerX = cWidth / 2 - 55;
graphContainerY = cHeight / 2 + 45;
graphContainerWidth = cWidth - 450;
...
function init() {
  ...
  currYTopVal = descValues[0] + descValues[0] * .25;
  currYMinVal = descValues[descValues.length - 1];
  ...
  verticalTicksContMask.width = graphContainerWidth + 130;
  ...
  horizontalGridsMask.position.set(0, 170);
  ...
  stopLimitX = -((perX(verticalGridGap) * (len - 2)) - 2100);
}
...
function update() {
  ...
  m = 0;
  if (frame > (350 * interval)) {
    m = frame - (350 * interval);
  }
  ...
  // move container
  if (itemCurrXPos >= graphContainer._width && moveContainerX == 1) {
  ...
  descValues = desc(values);
  tempTop = descValues[0] + descValues[0] * .25;
  tempMin = descValues[descValues.length - 1];

  if (tempMin < currYMinVal) {
    currYMinVal = tempMin;
    for (i in items) {
      items[i].valley();
    }
...
function verticalGrid() {
  ...
  this.draw = () => {
    ...
    this.tick.align = this.TextAlign;
    this.tick.letterSpacing = this.letterSpacing;
    this.tick.anchor.set(.5, 1);
    this.tick.position.set(this.pos, 0 - this.tickPad);
  }
  ...
  this.update();
  verticalTicksCont.addChild(this.grid);
...
function Item(arrY, name, icon, color) {
  ...
  this.imageCricle.drawCircle(0, 0, this.width / 2);
  this.imageCricle.endFill();
  lineContainer.addChild(this.imageCricle);
  lineContainer.setChildIndex(this.imageCricle, 0);
  ...
  // icon styles
  this.peakIcon = PIXI.Texture.from('img/p.png');
  this.peakIcon = new PIXI.Sprite(this.peakIcon);
  lineContainer.addChild(this.peakIcon);
  lineContainer.setChildIndex(this.peakIcon, 0);
  this.peakIcon.width = this.cricleRadius * 1.2;
  this.peakIcon.height = this.cricleRadius * 1.2;
  this.peakIcon.anchor.set(.5, .5);

  // this value text
  this.peakNumber = new PIXI.Text();
  this.peakNumber.fontFamily = 'nameFont';
  this.peakNumber.style._fontFamily = 'nameFont';
  this.peakNumber.font = 'nameFont';
  this.peakNumber.fontSize = 32;
  this.peakNumber.style.fontSize = 32;
  this.peakNumber.fill = '0xF44336';
  this.peakNumber._style.fill = '0xF44336';
  this.peakNumber.align = 'left';
  this.peakNumber.letterSpacing = 1;
  this.peakNumber.anchor.set(0, .5);
  this.peakNumber.text = '-' + numWithCommas(this.value);
  lineContainer.addChild(this.peakNumber);
  lineContainer.setChildIndex(this.peakNumber, 0);



  // icon styles
  this.valleyIcon = PIXI.Texture.from('img/v.png');
  this.valleyIcon = new PIXI.Sprite(this.valleyIcon);
  lineContainer.addChild(this.valleyIcon);
  lineContainer.setChildIndex(this.valleyIcon, 0);
  this.valleyIcon.width = this.cricleRadius * 1.2;
  this.valleyIcon.height = this.cricleRadius * 1.2;
  this.valleyIcon.anchor.set(.5, .5);

  // this value text
  this.valleyNumber = new PIXI.Text();
  this.valleyNumber.fontFamily = 'nameFont';
  this.valleyNumber.style._fontFamily = 'nameFont';
  this.valleyNumber.font = 'nameFont';
  this.valleyNumber.fontSize = 32;
  this.valleyNumber.style.fontSize = 32;
  this.valleyNumber.fill = '0x4CAF50';
  this.valleyNumber._style.fill = '0x4CAF50';
  this.valleyNumber.align = 'left';
  this.valleyNumber.letterSpacing = 1;
  this.valleyNumber.anchor.set(0, .5);
  this.valleyNumber.text = '-' + numWithCommas(this.value);
  lineContainer.addChild(this.valleyNumber);
  lineContainer.setChildIndex(this.valleyNumber, 0);
  ...
  this.icon.width = this.cricleRadius * 1.2;
  this.icon.height = this.cricleRadius * 1.2;
  ...
  lineContainer.setChildIndex(this.nameText, this.itemIndex + 1);



  // this value text
  this.valueNumber = new PIXI.Text();
  this.valueNumber.fontFamily = 'nameFont';
  this.valueNumber.style._fontFamily = 'nameFont';
  this.valueNumber.font = 'nameFont';
  this.valueNumber.fontSize = 32;
  this.valueNumber.style.fontSize = 32;
  ...
  this.valueNumber.anchor.set(0, .5);
  this.valueNumber.text = '-' + numWithCommas(this.value);
  lineContainer.addChild(this.valueNumber);
  lineContainer.setChildIndex(this.valueNumber, this.itemIndex + 1);
  ...
  this.setValue = (x) => {
    ...
    this.peakUpdateX(xPix);
    this.valleyUpdateX(xPix);
  }
  ...
  this.drawIcon = (x, y) => {
    ...
    this.icon.position.set(x + (this.icon.width / 2) + 10, y - 1);
  }
  ...
  this.drawNameValue = (x, y) => {
    ...
    this.valueNumber.position.set(x + this.cricleRadius + 32, y);
  }
  ...
  this.peak = () => {
    ...
    this.peakIcon.position.set(x + (this.icon.width / 2) + 10, y - 1);

    this.peakNumber.text = '-' + numWithCommas(Math.round(this.value));
    this.peakNumber.position.set(x + this.cricleRadius + 32, y);
    this.valleyPeak();

  }

  this.peakUpdateX = (x) => {
    this.peakIcon.position.x = (x + (this.icon.width / 2) + 10);
    this.peakNumber.position.x = (x + this.cricleRadius + 32);
  }
  ...
  this.valley = () => {
    ...
    this.valleyIcon.position.set(x + (this.icon.width / 2) + 10, y - 1);

    this.valleyNumber.text = '-' + numWithCommas(Math.round(currYMinVal));
    this.valleyNumber.position.set(x + this.cricleRadius + 32, y);

  }

  this.valleyUpdateX = (x) => {
    this.valleyIcon.position.x = (x + (this.icon.width / 2) + 10);
    this.valleyNumber.position.x = (x + this.cricleRadius + 32);
  }

  this.valleyPeak = () => {
    yPix = corY(perY(currYMinVal));

    y = yPix;
    this.valleyIcon.position.y = y - 1;
    this.valleyNumber.position.y = y;
  }
...
function displayDateFun(date) {
  if (firstDate == true) {
    ...
    runningDate.fill = '0x111111';
    runningDate._style.fill = '0x111111';
...
function showItemTop(index) {
  ...
  firstItemText.text = `O2 ${name}: -${numWithCommas(Math.round(value))}`;
  // console.log(index);
}
